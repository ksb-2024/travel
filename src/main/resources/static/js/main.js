...
function checkUser() {
   ...
   let idchecked = document.getElementById('idchecked').value;
...
$(document).ready(function(){
	$("#checkId").click(function(){
		...
		$.ajax({
			url:"/validateID/" + checkId,
			type:"GET",
			dataType:"text",
			success: function(data){
				//console.log("data : " + data);
				if(data == 'OK'){
					$('#message_id1').css('display', 'none');
					$('#message_id4').css('display', 'block');
					idchecked.val(1);
				}
				else {
					$('#message_id1').css('display', 'block');
					$('#message_id4').css('display', 'none');
				}
			},
			error : function(){
